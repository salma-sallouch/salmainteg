...
on_button13_clicked                    (GtkButton       *button,
                                        gpointer         user_data)
{
    GtkWidget *current_window = GTK_WIDGET(user_data);  // Assuming the current window is passed as user_data.

    // Destroy the current window
    gtk_widget_destroy(current_window);

    // Create the new "edit profile" window
    GtkWidget *nca =   create_Espace_client ();

    // Show the new window
    gtk_widget_show_all(nca);
}
